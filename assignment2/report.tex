...
assigned to the thread, and then splitting the remaining memory between the two
threads that process left and right branches:
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \matrix (A) [matrix of nodes, nodes={draw, minimum size=8mm},
...
\ref{sec:next_rank}). Therefore we write $w(P,N) = \alpha(P)t(N)$ where
...
\end{gather*}
If we use OpenMp there is no merging needed (unless we employ the approach
...
level of 5 or 6 workers is practically not the same of waiting for the
computation of the additional level of 1 worker, since in the former case the
probability of having a slower worker is much higher. The slower worker is a
\emph{bottleneck}, therefore increasing the number of workers (even though we
do not reach the next power of 2) reduces the probability of having a
...
inspection of the simplified model (i.e. steep improvement everytim we add $2^k$
processors). Also, we observe that the hidden cost of OpenMP is definitely
...
Note that in this case we observe the effect we forecasted (and explained) in
the last paragraph of the introduction to Section \ref{sec:performance}, i.e.
...
a set $\mathcal{D} = \{d_1.csv, \dots, d_24.csv\}$ comprising 24 datasets having
varying sizes, such that \texttt{len}$(d_i) = i * 1000000$. With respect to
\cite[Chapter~5.3.2]{hager2010introduction} we choose $\alpha = 1$. Then we
...
\subsection{Other performance experiments}
\begin{figure}[t!]
    \centering
    \begin{subfigure}[b]{0.4\textwidth}
        \centering
...
\begin{figure}[b!]
    ...
    \begin{subfigure}[b]{0.4\textwidth}
        ...
        \end{tikzpicture}
    \end{subfigure}
    \caption{Time measured for the growth of a \kdtree{} ($N = $ 100'000'000).}
...
when MPI is used: this is most likely due to the fact that communication cost
...
\begin{figure}[t!]
    ...
    \begin{tikzpicture}[scale=0.8]
        ...
        \addplot table[x index=0, y index=2] {plots_data/false_sharing.txt};
