...
\caption{Strong scalability of the growth of a \kdtree{} ($N = $ 100'000'000, $k = $ 3).}
...
\begin{figure}[b!]
    ...
    \begin{subfigure}[b]{0.4\textwidth}
        ...
        \end{tikzpicture}
    \end{subfigure}
    \caption{Weak scalability of the growth of a \kdtree{} for a linearly varying number of elements ($k = 3$).}
...
In this small section we gather several additional interesting experiments along
with a brief discussion of the results.

\subsubsection{Dependency on the number $k$ of components}
In this experiment we observe the dependency of the time measured during
the growth of a \kdtree{} with a varying number of components $k$ ($N$ is fixed
to 1000000). We also consider several numbers of parallel workers for both
the MPI and OpenMP versions. The plot on the left of Figure
\ref{fig:n_components} shows that the MPI version presents an approximately
linear dependency of the time needed to grow the tree on the number of
components in each data point, as the number of MPI processes involved increases
(we have some unstable measurements when 2 processes are used). The OpenMP
version by contrast is much more stable when a small number of OMP threads is
used, and we can observe a less steep linear
dependency on the number of components $k$. Note that since our parallel
algorithm is agnostic on the number of components (i.e. in theory we do not
expect any particular overhead when the number of components increases) the
observed increased cost for big values of $k$ is entirely due to memory
management issues (like \emph{false sharing}, moving memory, cache coherency,
message passing).
    ...
    \begin{subfigure}[b]{0.5\textwidth}
        ...
        \begin{tikzpicture}[scale=0.75]
            \begin{axis}[
                ...
                \addplot3[surf,mesh/rows=5] file {plots_data/varying_n_components_mpi.txt};
            \end{axis}
        \end{tikzpicture}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.5\textwidth}
        ...
        \begin{tikzpicture}[scale=0.75]
            \begin{axis}[
                ...
                \addplot3[surf,mesh/rows=5] file {plots_data/varying_n_components_omp.txt};
            \end{axis}
        \end{tikzpicture}
    \end{subfigure}
    }
    \caption{Performance measured for the growth of \kdtree{} with
    $N = $ 1000000, varying the number of components $k$.}
    \label{fig:n_components}
\end{figure}

\subsubsection{Compiler optimzation}
\begin{figure}[t!]
    \centering
    \begin{subfigure}[b]{0.4\textwidth}
        \centering
    ...
    \begin{subfigure}[b]{0.4\textwidth}
        ...
        \end{tikzpicture}
    \end{subfigure}
    \caption{Performance determined by compilation with different compiler optimization flags ($N = $ 100'000'000, $k = $ 3).}
...
compilations. The fact that the plots for higher optimization levels are
almost overlying is probably due to the fact that the most computational
intensive operations in our algorithm are the calls to
\texttt{std::nth\_element} and memory operations, which are only slightly
influenced by the compiler. We might be able to see more distinct lines if we
consider smaller datasets and a small number of processors.

\subsubsection{\texttt{double} VS \texttt{float}}
\begin{figure}[b!]
    \centering
    \begin{subfigure}[b]{0.4\textwidth}
        \centering
    ...
    \begin{subfigure}[b]{0.4\textwidth}
        ...
        \end{tikzpicture}
    \end{subfigure}
    \caption{Time taken by the growth of a \kdtree{} ($N = $ 100'000'000, $k = 3$).}
...
\begin{figure}[b!]
    ...
    \begin{tikzpicture}[scale=0.8]
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Results obtained applying the fix for false sharing presented in Section \ref{sec:false_sharing} ($N = $ 100'000'000, $k = $ 3).}
...
\begin{figure}[t!]
    ...
    \begin{subfigure}[b]{0.4\textwidth}
        ...
        \begin{tikzpicture}[scale=0.75]
            ...
            \addplot table[x index=0, y index=1] {plots_data/big_dataset_gpu.txt};
            \addplot table[x index=0, y index=2] {plots_data/big_dataset_gpu.txt};
    ...
    \begin{subfigure}[b]{0.4\textwidth}
        ...
        \begin{tikzpicture}[scale=0.75]
            ...
            \addplot table[x index=0, y index=3] {plots_data/big_dataset_gpu.txt};
            \addplot table[x index=0, y index=4] {plots_data/big_dataset_gpu.txt};

            \legend{MPI, OpenMP}
            \end{axis}
        \end{tikzpicture}
    \end{subfigure}
\caption{Strong scalability of the growth of a \kdtree{} on a single GPU node ($N = $ 100'000'000, $k = $ 3).}
\label{fig:strong_scaling_gpu}
\end{figure}
\begin{figure}[b!]
    \centering
    \begin{tikzpicture}[scale=0.75]
        \begin{axis}[
            ...
            xlabel={$P/2$},
            legend cell align={left},
            title={Time needed to grow the \kdtree{} (MPI).},
            legend style={at={(0.5,0.6)},anchor=west},
        ]
        \addlegendimage{empty legend}
        \addplot table[x index=0, y index=1] {plots_data/mapby_gpu.txt};
        \addplot table[x index=0, y index=2] {plots_data/mapby_gpu.txt};
        \addplot table[x index=0, y index=3] {plots_data/mapby_gpu.txt};

        \addlegendentry{\hspace{-.6cm}\texttt{--map-by}}
        \addlegendentry{core}
        \addlegendentry{socket}
        \addlegendentry{node}
        \end{axis}
    \end{tikzpicture}
\caption{Time mesured adding incrementally pairs of processors on 2 GPU nodes, i.e. 2,4,\dots,46,48 ($N = $ 100'000'000, $k = $ 3).}
\label{fig:mapby_gpu}
\end{figure}
In Figure \ref{fig:strong_scaling_gpu} we plot the performance measured on a single
GPU node going all the way from 1 to 48 CPUs. As you can see the overall
performance is worse than what we measured on THIN nodes, so much that the
lowest time measured is still above 20 seconds for the same dataset comprising
100'000'000 data points which we considered in Figure \ref{fig:strong_scaling}.
At $P=25$ the MPI version presents a steep descent, exactly when as we go
beyond the number of processors on a single node. From this point the MPI
performance becomes approximately a plateau, most likely due to the fact that
adding new MPI processes does not guarantee any particular gain due to the
additional communication overhead. In order to clarify the situation we present
the plot in Figure \ref{fig:mapby_gpu}, where we summarized the behavior of the
MPI version for different usages of the parameter \texttt{--map-by} of
\texttt{mpirun}. In this experiment we incrementally add two processors from two
different GPU nodes for each measurement; then we measure the performance in
the three cases (\texttt{core}, \texttt{socket}, \texttt{node}). As expected the
worst performance overall is the one measured for \texttt{node}, since in this
case we have the maximum distance for the communication between MPI processes.
