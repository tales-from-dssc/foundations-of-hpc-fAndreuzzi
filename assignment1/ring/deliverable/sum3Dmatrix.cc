...
int main(int argc, char **argv) {
  ...
  for (int iter = 0; iter < ITERATIONS; iter++) {
    ...
    if (rank == 0) {
      ...
      std::cout << "Max error: " << max_error << " on iteration " << iter
                << std::endl;
    }
#endif
  }
...
#else
    double mean_tot = 0, mean_scatter = 0, mean_gather = 0;
    for (int i = 0; i < ITERATIONS; i++) {
      mean_tot += tot_time[i] / ITERATIONS;
      mean_scatter += scatter_time[i] / ITERATIONS;
      mean_gather += gather_time[i] / ITERATIONS;
    }
    std::cout << mean_tot << "," << mean_scatter << "," << mean_gather
              << std::endl;
#endif
  }
